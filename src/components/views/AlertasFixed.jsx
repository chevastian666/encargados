...
/**
 * Modal de Alertas Operativas optimizado para tablet (CORREGIDO)
 */
const AlertasFixed = ({ isOpen, onClose, darkMode }) => {
  ...
  // Configuración de tipos con clases fijas
  const tiposAlerta = {
    operativa: { 
      icon: AlertTriangle, 
      label: 'Operativa',
  ...
  // Configuración de prioridades con clases fijas
  const prioridades = {
    alta: { 
      label: 'Alta',
  ...
  const AlertaCard = ({ alerta }) => {
    const tipo = tiposAlerta[alerta.tipo] || tiposAlerta.operativa;
    const prioridad = prioridades[alerta.prioridad] || prioridades.media;
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-3">
                <div className={`p-3 rounded-lg ${tipo.bgClass}`}>
                  <TipoIcon className={`w-6 h-6 ${tipo.iconClass}`} />
                </div>
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${prioridad.bgClass}`}>
